...
namespace MaxLifxCoreBulbController
{
    public class ExtendedMultizonePayloadCache
    {
        ...
        public void Send(Dictionary<string, System.Net.Sockets.UdpClient> reusableHomebrewClientDictionary, MaxLifxCoreBulbController.Controllers.MaxLifxCoreBulbController bulbController)
        {
            if (Payloads.Any())
            {
                        ...
                        if (ctr % 256 == 255 || ctr == group.Count() - 1)
                        {
                            if (!reusableHomebrewClientDictionary.ContainsKey(group.Key.IpAddress))
                            {
                                reusableHomebrewClientDictionary.Add(group.Key.IpAddress,
                                    MaxLifxCoreBulbController.Controllers.MaxLifxCoreBulbController.GetPersistentClient(group.Key.MacAddress, group.Key.IpAddress));
                            }
